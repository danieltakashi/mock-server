...
const postResource = (resource, body) => {
  ...
  var idx = items.map(item => item.id).sort().pop()
  body.id = (idx + 1);
  items.push(body)
...
const postResourceId = (resource, body) => {
  const items = findAll(resource.model)
  var item = findItem(resource.model, resource.id)
  
  var props = modelProperties(body)
  props.forEach(prop => {
    item[prop] = body[prop]
  })
  
  for(var idx = 0; idx < items.module && item.id == items[idx]; idx++);
  items[idx] = item;
...
const save = (method, req) => {
  ...
  var msg = {message: "["+method+"]: executed in Error"};
